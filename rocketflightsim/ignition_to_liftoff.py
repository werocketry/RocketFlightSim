...
def flight_sim_ignition_to_liftoff(rocket, launch_conditions):
    ...
    liftoff_thrust_to_mass_ratio = launch_conditions.liftoff_thrust_to_mass_ratio
    ...
    # find the first time when thrust exceeds lifroff thrust, or raise an exception if the rocket never lifts off
    try:
        post_liftoff_key = next(key for key, thrust in engine_thrust_lookup.items() if thrust > liftoff_thrusts[key])
    except StopIteration:
        raise Exception("The motor never produces enough thrust for the rocket to liftoff")
