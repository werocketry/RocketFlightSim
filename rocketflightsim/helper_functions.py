...
def air_density_optimized(temp, multiplier, exponent):
    ...
    ----
    temp : float
        Temperature at the given height in Kelvin.
    multiplier : float
        A constant derived from the temperature and pressure at the launchpad, the lapse rate, the specific gas constant for air, and the magnitude of the force of gravity. Calculated at initialization of LaunchConditions objects. Equal to P_launchpad / (R_air * pow(T_launchpad, - F_gravity / (R_air * T_lapse_rate))).
    exponent : float
        A constant derived from the lapse rate, the specific gas constant for air, and the magnitude of the force of gravity. Calculated at initialization of LaunchConditions objects. Equal to - F_gravity / (R_air * T_lapse_rate) - 1.
...
def get_local_gravity(latitude, h = 0, h_expected = None):
    ...
    TODO: split off the h_expected thing into its own function
    Args
    ----
    latitude : float
    ...
    h_expected : float
        Expected altitude the rocket will reach above sea level in meters. Defaults to None, in which case the function will return the acceleration due to gravity at the launchpad instead of the average between the launchpad and the expected apogee.
...
def pressure_at_altitude(h, launchpad_temp, launchpad_pressure):
    ...
    ----
    h : float
        Altitude above the launchpad in meters.
    launchpad_temp : float
        Temperature at the launchpad in Kelvin.
    launchpad_pressure : float
        Air pressure at the launchpad in Pascals.
