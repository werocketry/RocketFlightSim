...
class TestNewSimulation(unittest.TestCase):
    def test_new_simulation(self):
        ...
        from rocketflightsim.flight_simulation import flight_sim_ignition_to_liftoff, flight_sim_liftoff_to_rail_clearance, flight_sim_rail_clearance_to_burnout, flight_sim_burnout_to_apogee
        ...
        for past_flight in past_flights:
            ...
            new_rail_clearance_function_states = flight_sim_liftoff_to_rail_clearance(past_flight.rocket, past_flight.launch_conditions, new_time_to_liftoff)

            print(f"With new liftoff to rail clearance function, the state at rail clearance is:\n {new_rail_clearance_function_states[-1]}")

            new_till_burnout_function_states = flight_sim_rail_clearance_to_burnout(past_flight.rocket, past_flight.launch_conditions, new_rail_clearance_function_states[-1])
            
            print(f"With new rail clearance to burnout function, the state at burnout is:\n {new_till_burnout_function_states[-1]}")

            new_till_apogee_function_states = flight_sim_burnout_to_apogee(past_flight.rocket, past_flight.launch_conditions, new_till_burnout_function_states[-1])
